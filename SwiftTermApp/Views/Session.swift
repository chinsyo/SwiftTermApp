...
class Session: CustomDebugStringConvertible, Equatable {
    ...
    public init(host: Host, delegate: SessionDelegate, send: @escaping SocketCallback, recv: @escaping SocketCallback, disconnect: @escaping DisconnectCallback, debug: @escaping DebugCallback) {
...
class SocketSession: Session {
    ...
    public init(host: Host, delegate: SessionDelegate) {
        let send: SocketCallback = { socket, buffer, length, flags, abstract in
            let n = SocketSession.sendCallback(socket: socket, buffer: buffer, length: length, flags: flags, abstract: abstract)
            if debugIO {
        ...
        let recv: SocketCallback = { socket, buffer, length, flags, abstract in
            let n = SocketSession.recvCallback(socket: socket, buffer: buffer, length: length, flags: flags, abstract: abstract)
            if debugIO {
                print ("SocketSession.recv: \(n)")
            }
            return n
        }
        let disconnect: DisconnectCallback = { sess, reason, message, messageLen, language, languageLen, abstract in
        ...
        let debug: DebugCallback = { session, alwaysDisplay, messagePtr, messageLen, languagePtr, languageLen, abstract in
    ...
    static func sendCallback(socket: libssh2_socket_t, buffer: UnsafeRawPointer, length: size_t, flags: CInt, abstract: UnsafeRawPointer) -> ssize_t {
        let session = SocketSession.getSocketSession(from: abstract)
    ...
    static func recvCallback(socket: libssh2_socket_t, buffer: UnsafeRawPointer, length: size_t, flags: CInt, abstract: UnsafeRawPointer) -> ssize_t {
        let session = SocketSession.getSocketSession(from: abstract)
